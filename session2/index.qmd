...
## 🧩 Conditional Code Execution by Output Format

Sometimes you may want to run a code chunk **only when rendering to HTML** or **only for PDF** output.

### ✅ Run Only for HTML Output

Use the `eval` option with `knitr::is_html_output()`:
...
### ✅ Run Only for PDF Output

Use `knitr::is_latex_output()`:
...
* 🧩 **Conditional Rendering** – You can control which code chunks run for HTML vs. PDF using `knitr::is_html_output()` or `knitr::is_latex_output()`.
